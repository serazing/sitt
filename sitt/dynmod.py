...
def dynmodes(Nsq, depth, nmodes):
	...
	eigenvalues_p, eigenvalues_w, pmodes, wmodes = (
		eigenvalues_p[:nmodes], eigenvalues_w[:nmodes], pmodes[:, :nmodes],
		wmodes[:, :nmodes])
	...
	print
	"Mode speeds do correspond: %s" % np.allclose(ce * np.sqrt(eigenvalues_w),
	                                              1.)
...
def build_d2dz2_matrix_p(z, Nsq):
	...
	diags = np.vstack((d0, d1, dm1))
	d2dz2 = scipy.sparse.dia_matrix((diags, (0, 1, -1)),
	                                shape=(len(z) - 1, len(z) - 1))
	return d2dz2, dz
